    ...
    <div className="max-w-2xl mx-auto pb-24">
      ...
      <div className="mt-8">
        <button
          type="button"
          onClick={() => {
            try { localStorage.removeItem('duo-onboarding-v1') } catch { /* ignore */ }
            window.location.reload()
          }}
          className="duo-btn duo-btn-white w-full sm:w-auto px-6 py-3 text-sm"
        >
          Redo intro questions
        </button>
        <p className="text-xs ink-3 font-bold mt-2">Shows the first-time setup questions again (language, goal, etc.).</p>
      </div>
    </div>
  )
}
