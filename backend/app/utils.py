...
def check_answer(exercise_payload: dict, exercise_type: str, answer) -> bool:
    if exercise_type in ('multiple_choice', 'fill_blank', 'follow_pattern'):
...
COURSE_META = {
    'es': {'id': 'spanish-101', 'language': 'Spanish', 'languageCode': 'es', 'flag': '🇪🇸'},
    'math': {'id': 'math-101', 'language': 'Math', 'languageCode': 'math', 'flag': '🔢'},
}


def is_math_unit(unit_id: str) -> bool:
    return unit_id.startswith('mu')


def filter_units_for_course(units, language: str):
    """Return units belonging to the active course (Spanish or Math)."""
    want_math = language == 'math'
    return [u for u in units if is_math_unit(u.id) == want_math]
