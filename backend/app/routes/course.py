...
from ..utils import get_or_create_user, COURSE_META, filter_units_for_course
...
async def list_courses():
    """Available courses (Spanish + Math)."""
    return {
        'courses': [
            {**COURSE_META['es'], 'description': 'Learn Spanish with bite-sized lessons'},
            {**COURSE_META['math'], 'description': 'Practice addition, patterns & more'},
        ]
    }
...
async def get_course(
    course: str | None = Query(None, description='es | math — defaults to user.language'),
    session: AsyncSession = Depends(get_session),
):
    user = await get_or_create_user(session)
    lang = course if course in COURSE_META else (user.language if user.language in COURSE_META else 'es')
    ...
    units = filter_units_for_course(units, lang)
    ...
    for u in units:
        ...
        for s in sorted(u.skills, key=lambda x: x.order_index):
            ...
            finished = lessons_completed >= total_lessons and total_lessons > 0
